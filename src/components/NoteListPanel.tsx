...
export default function NoteListPanel({
  ...
  // ── Infinite scroll sentinel (per-contact panel still uses this) ──────────
  ...
  const updateScroll = useCallback(() => {
    ...
    if (!scrollable) { setScrollProgress(0); setIsAtTop(true); setIsAtBottom(true); return; }
    const progress = el.scrollTop / max;
    setScrollProgress(progress);
    setIsAtTop(el.scrollTop <= 0);
    setIsAtBottom(el.scrollTop >= max - 1);
  }, []);
  ...
  useEffect(() => {
    ...
    // Re-measure whenever notes list changes length
    updateScroll();
  ...
  const handleThumbMouseDown = (e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      const trackEl = scrollRef.current.parentElement?.querySelector<HTMLDivElement>("[data-scroll-track]");
      if (!trackEl) return;
  ...
  // Approximate visible-note-index from scroll position
  const visibleIndex = Math.max(1, Math.round(scrollProgress * notes.length));
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        ...
        {/* ── Custom scroll rail ──────────────────────────────────────────────
            Visible only when the list is scrollable. Collapses to a 3px track
            at rest; expands to 28px on hover revealing the thumb, position
            counter, and jump arrows.
        ─────────────────────────────────────────────────────────────────────── */}
        {canScroll && !isLoading && (
          <div
          ...
          >
            ...
            >
              ...
              <div style={{
                position: "absolute", top: 0, bottom: `${(1 - scrollProgress) * 100}%`,
                width: "2px", borderRadius: "1px",
                background: "linear-gradient(to bottom, #667eea, #764ba2)",
                left: "50%", transform: "translateX(-50%)",
                transition: isDragging.current ? "none" : "bottom 0.08s",
              }} />
              ...
              <div
                ...
                style={{
                  ...
                  borderRadius: "4px",
                  cursor: "grab",
                  transition: isDragging.current ? "none" : "top 0.08s, width 0.2s",
