...
// Format for the text input: "05/11/2026"
...
// Parse what the user typed → "YYYY-MM-DD" or null
...
export default function DatePicker({
  ...
  const openPicker = () => {
    ...
    // Sync calendar view to selected date on open
  ...
  const handleInputFocus = () => {
    setDraftText(toInputText(value)); // prefill with current value for editing
    openPicker();
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (parsed) {
      onChange(parsed);
      // Navigate calendar to the typed month
      const p = parseDateValue(parsed);
      if (p) { setViewYear(p.year); setViewMonth(p.month); }
    }
  };

  const handleInputBlur = () => {
    setDraftText(null); // revert to formatted display
  };
      ...
      {isOpen && (
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
            ...
            <span style={{ fontSize: "14px", fontWeight: 700, color: colors.text, letterSpacing: "0.01em" }}>
            ...
            {WEEKDAYS.map(d => (
              <div
                ...
                style={{
                  ...
                  color: colors.textSecondary, padding: "3px 0",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "2px" }}>
            {cells.map(({ date, isCurrentMonth }) => {
              ...
              const hovered = hoveredDay === key && !selected;
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: todayMark && !selected ? "1.5px solid #667eea" : "1.5px solid transparent",
                    borderRadius: "7px",
