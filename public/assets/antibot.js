 ...
 *
 * Wrapped in an IIFE so its declarations are function-scoped, not global. This
 * makes the script safe to inject more than once on the same page (e.g. when the
 * background worker re-injects into an already-open tab): a classic script with
 * a top-level `const` would throw "Identifier has already been declared" on the
 * second injection, but function-scoped bindings simply shadow and no-op.
 */
...
(function () {
  ...
  // context and has no access to chrome.runtime. Read synchronously here.
  const _extIndexUrl =
  ...
  // The `fetchReplaced` flag also guarantees we never double-wrap fetch.
